...
Function FileText(FileName$) As String
    ...
    Close #handle
End Function
Function SaveFileText(FileName$, Texte As String) As String
    Dim handle As Integer
    handle = FreeFile
...
Dim n As Integer
  ...
  If Len(Trim$(Player(Index).Login)) <= 1 Then Exit Sub
    ...
    For i = 1 To 3
        ...
        For n = 1 To 70
        Call PutVar(FileName, "CHAR" & i, "Flag" & n, Trim$(Player(Index).Char(i).Flag(n)))
        Next n
        ...
        Call PutVar(FileName, "CHAR" & i, "Mobs", STR$(Player(Index).Char(i).mobs))
        Call PutVar(App.Path & "\classement.ini", "classement", Trim$(Player(Index).Char(i).Name), STR$(Player(Index).Char(i).mobs))
...
With Player(Index)
    ...
    With .Char(i)
        ' General
        .ID = GetVar(FileName, "CHAR" & i, "ID")
        For n = 1 To 70
        .Flag(n) = GetVar(FileName, "CHAR" & i, "Flag" & n)
        Next n
        ...
        If Not FileExist("\Guildes\" & GetVar(FileName, "CHAR" & i, "Guild") & ".ini") And GetVar(FileName, "CHAR" & i, "Guild") <> "" Then Call MapPlayerMsg(Index, "Votre guilde n'existe plus!", vbRed): GoTo suite
        .Guild = GetVar(FileName, "CHAR" & i, "Guild")
        .Guildaccess = Val(GetVar(FileName, "CHAR" & i, "Guildaccess"))
suite:
        .mobs = GetVar(FileName, "CHAR" & i, "Mobs")
...
suite:
    Next vLine
packet = "classement" & SEP_CHAR & "Top 1 : " & Top(1).nom & " avec " & Top(1).mobs & " monstres."
packet = packet & SEP_CHAR & "Top 2 : " & Top(2).nom & " avec " & Top(2).mobs & " monstres."
packet = packet & SEP_CHAR & "Top 3 : " & Top(3).nom & " avec " & Top(3).mobs & " monstres." & END_CHAR
...
suite2:
    Next vLine
packet = "classementgvg" & SEP_CHAR & "Top 1 : " & TopGvG(1).nom & " avec " & TopGvG(1).mobs & " Points."
packet = packet & SEP_CHAR & "Top 2 : " & TopGvG(2).nom & " avec " & TopGvG(2).mobs & " Points."
packet = packet & SEP_CHAR & "Top 3 : " & TopGvG(3).nom & " avec " & TopGvG(3).mobs & " Points." & END_CHAR
SendDataToAll (packet)
End Sub
Public Sub GuildUpdate(ByVal Index As Long)
...
ListeSection FileName, Section
        ...
        For i = LBound(Section) To UBound(Section)
            packet = packet & ReadINI(Section(i), "nom", FileName) & SEP_CHAR & ReadINI(Section(i), "rang", FileName) & SEP_CHAR
        Next i
