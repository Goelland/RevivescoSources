...
Public clsScriptCommands2 As clsCommands
...
Dim f As Long
    ...
    frmServer.WinEdit(0).RemoteHost = frmServer.WinEdit(0).LocalIP
    frmServer.WinEdit(0).RemoteHost = "4001"
    ...
    For i = 1 To MAX_PLAYERS
        ...
        Load frmServer.WinEdit(i)
        Call ShowPLR(i)
    Next i
    ReDim Editeur(0 To MAX_PLAYERS) As EditeurRec
    loading (15)
    'For i = 1 To MAX_PLAYERS
    '    Call ShowPLR(i)
    'Next i
...
On Error Resume Next
    ...
    For i = 1 To MAX_PLAYERS
        ...
        If frmServer.WinEdit(i).State > 7 Then Call CloseWinEdit(i)
    Next i
...
Dim Parse() As String, Answer As Integer
    ...
    Select Case LCase$(Parse(0))
        Case "help", "?", "aide"
           Call MapPlayerMsg(Index, " ---------- Aide Hotel de Ventes ---------- ", White)
           Call MapPlayerMsg(Index, " Acheter -> /hdvs achat", White)
           Call MapPlayerMsg(Index, " Vendre  -> /hdvs vente", White)
           Call MapPlayerMsg(Index, " Annuler ->", White)
           Call MapPlayerMsg(Index, " . Achat -> /hdvs sachat", White)
           Call MapPlayerMsg(Index, " . Vente -> /hdvs svente", White)
        Case "achat", "vente"
    ...
    End Select
End Sub
Public Sub InitNpcScript()
Dim i As Long
With frmServer
        .NPCScript.Reset
         Set clsScriptCommands2 = New clsCommands
        .NPCScript.AllowUI = True
        .NPCScript.AddObject "clsScriptCommands2", clsScriptCommands2, True
        On Error GoTo trap
            For i = 1 To 300
                '.NPCScript.Modules.add ("NPC" & i)
                If FileExist("\Npcs\Npc" & i & ".txt") Then
                    .NPCScript.AddCode (Trim(FileText(App.Path & "\Npcs\NPC" & i & ".txt")))
                Else
                    .NPCScript.AddCode "Public Sub NPC" & i & "(byval index, byval texte)" & vbNewLine & "End Sub"
                    'MsgBox .NPCScript.Modules("NPC" & i).Name
                End If
trap:
            Next i
End With
