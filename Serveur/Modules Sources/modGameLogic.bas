    ...
    For i = 1 To MAX_INV
        ...
        If GetPlayerInvItemNum(Index, i) = ItemNum Then
            ...
            Else
                ...
                If (n <> ITEM_TYPE_WEAPON) And (n <> ITEM_TYPE_ARMOR) And (n <> ITEM_TYPE_HELMET) And (n <> ITEM_TYPE_SHIELD) Then TakeItem = True
            End If
        ...
        If (MapItem(MapNum, i).Num > 0) And (MapItem(MapNum, i).Num <= MAX_ITEMS) Then
            ...
            If (MapItem(MapNum, i).x = GetPlayerX(Index)) And (MapItem(MapNum, i).y = GetPlayerY(Index)) Then
                ...
                If n <> 0 Then
                   ...
                   Call MapPlayerMsg(Index, Msg, Yellow)
                    Exit Sub
                Else
                   Call MapPlayerMsg(Index, "Votre inventaire est plein.", BrightRed)
                    Exit Sub
...
Dim i As Long
    ...
    If (GetPlayerInvItemNum(Index, InvNum) > 0) And (GetPlayerInvItemNum(Index, InvNum) <= MAX_ITEMS) Then
        ...
        Else
           Call MapPlayerMsg(Index, "Il y a trop d'objets par terre .", BrightRed)
        End If
    End If
End Sub
                    ...
                    If Map(GetPlayerMap(Victim)).Tile(GetPlayerX(Victim), GetPlayerY(Victim)).type <> TILE_TYPE_ARENA And Map(GetPlayerMap(Attacker)).Tile(GetPlayerX(Attacker), GetPlayerY(Attacker)).type <> TILE_TYPE_ARENA Then
                        ...
                        If Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NONE Or Map(GetPlayerMap(Attacker)).Moral = MAP_MORAL_NO_PENALTY Or GetPlayerPK(Victim) = YES Then
                            ...
                            If Trim$(GetPlayerGuild(Attacker)) <> vbNullString And GetPlayerGuild(Victim) <> vbNullString Then
                                ...
                                Else
                                   Call MapPlayerMsg(Attacker, "Vous ne pouvez pas attaquer un membre de votre guilde .", BrightRed)
                                End If
                            Else
                                CanAttackPlayer = True
                            End If
                        Else
                           Call MapPlayerMsg(Attacker, "C'est une safe zone (impossible d'attaquer d'autres joueurs).", BrightRed)
                        End If
...
If Map(GetPlayerMap(Attacker)).Tile(GetPlayerX(Attacker), GetPlayerY(Attacker)).type <> TILE_TYPE_ARENA And Map(GetPlayerMap(Victim)).Tile(GetPlayerX(Victim), GetPlayerY(Victim)).type <> TILE_TYPE_ARENA Then
    If Damage >= GetPlayerHP(Victim) Then
        ...
        ' attaquant a une guilde, ajouter au classement
        If Not GetPlayerGuild(Attacker) = "" Then Call PutVar(App.Path & "\classement.ini", "classementgvg", Trim$(GetPlayerGuild(Attacker)), Val(GetVar(App.Path & "\classement.ini", "classementgvg", GetPlayerGuild(Attacker))) + 1)
...
'    Stop
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_HEAL Then
        ...
        Call SendHP(Index)
       Call MapPlayerMsg(Index, "Tu sens ta force revenir peu a peu.", BrightGreen)
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_KILL Then
       ...
       Call MapPlayerMsg(Index, "Tu sens la mort arriver et tu perds peu a peu tes forces.", BrightRed)
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_SHOP Then
       If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data1 > 0 Then
            ...
            End If
        Else
           Call MapPlayerMsg(Index, "Il n'y a pas de magasin ici.", BrightRed)
        End If
    End If
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_SPRITE_CHANGE Then
        If GetPlayerSprite(Index) = Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data1 Then
           Call MapPlayerMsg(Index, "Tu as déjà ce sprites.", BrightRed)
            Exit Sub
        Else
            ...
            Else
                If item(Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data2).type = ITEM_TYPE_CURRENCY Then
                   Call MapPlayerMsg(Index, "Ce sprite vous coûte " & Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data3 & " " & Trim$(item(Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data2).Name) & ".", Yellow)
                    ...
                    Call SendInventory(Index)
                Else
                   Call MapPlayerMsg(Index, "Ce sprite vous coûte un " & Trim$(item(Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data2).Name) & ".", Yellow)
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_CLASS_CHANGE Then
        If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data2 > -1 Then
            If GetPlayerClass(Index) <> Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).data2 Then
               Call MapPlayerMsg(Index, "Tu n'as pas la classe requise.", BrightRed)
                Exit Sub
            End If
        End If
        ...
        Else
           ...
           Call MapPlayerMsg(Index, "Ta nouvelle classe est " & Trim$(Classe(GetPlayerClass(Index)).Name) & ".", BrightGreen)
    ...
    If Map(GetPlayerMap(Index)).Tile(GetPlayerX(Index), GetPlayerY(Index)).type = TILE_TYPE_CRAFT Then
        If Player(Index).Char(Player(Index).charnum).metier > 0 Then
            If metier(Player(Index).Char(Player(Index).charnum).metier).type = METIER_CRAFT Then
                ...
                Call SendDataTo(Index, packet)
            Else
               Call MapPlayerMsg(Index, "Votre métier n'est pas un métier de craft.", Red)
            End If
        Else
           Call MapPlayerMsg(Index, "Vous n'avez pas de métier .", Red)
        End If
    End If
...
Dim f As Long
    ...
    Else
        ...
        ' Send them welcome
       Call MapPlayerMsg(Index, "Bienvenue sur " & GAME_NAME & ".", 15)
        
        ' Send motd
        If Trim$(MOTD) <> vbNullString Then Call MapPlayerMsg(Index, "MOTD: " & MOTD, 11)
    End If
...
'                    If n <> Index Then
                        ...
                        Else
                            If GetPlayerMap(Index) = GetPlayerMap(n) And Spell(SpellNum).type >= SPELL_TYPE_ADDHP And Spell(SpellNum).type <= SPELL_TYPE_ADDSP Or Spell(SpellNum).type = SPELL_TYPE_SCRIPT Then
                                ...
                                Casted = True
                            Else
                               Call MapPlayerMsg(Index, "Vous n'avez pas pu envoyer le sort.(la cible n'est pas sur la même carte que vous)", BrightRed)
                            End If
                        End If
                    'Else
                      '  Player(Index).TargetType = TARGET_TYPE_PLAYER
...
Else
    n = Player(Index).Target
    If n = -1 Then Call MapPlayerMsg(Index, "Vous n'avez pas pu envoyer le sort.(aucune cible)", BrightRed): Exit Sub
    If Player(Index).TargetType = TARGET_TYPE_PLAYER Then
        If IsPlaying(n) Then
            If bouclier(n) = True Then
                Call BattleMsg(Index, "Le sort ne peut pas toucher le joueur car il a un bouclier.", BrightRed, 0)
            ...
            End If
        Else
           Call MapPlayerMsg(Index, "Vous n'avez pas put envoyer le sort.(cible hors ligne)", BrightRed)
        End If
    Else
        If CInt(Sqr((GetPlayerX(Index) - MapNpc(GetPlayerMap(Index), n).x) ^ 2 + ((GetPlayerY(Index) - MapNpc(GetPlayerMap(Index), n).y) ^ 2))) > Spell(SpellNum).Range Then
            Call BattleMsg(Index, "Vous êtes trop loin pour toucher la cible.", BrightRed, 1)
            Exit Sub
        End If
...
If Npc(npcnum).Behavior = NPC_BEHAVIOR_FRIENDLY Or Npc(npcnum).Behavior = NPC_BEHAVIOR_SHOPKEEPER Or Npc(npcnum).Behavior = NPC_BEHAVIOR_QUETEUR And Npc(npcnum).Behavior = NPC_BEHAVIOR_SCRIPT Then
    If Npc(npcnum).Behavior = NPC_BEHAVIOR_QUETEUR And ACoter(MapNpcNum, Attacker) Then Call MapPlayerMsg(Attacker, "Ne pointe pas cette arme sur moi si tu veut me parler.", BrightRed)
    CanAttackNpcWithArrow = False
    Exit Function
End If
   ...
   If npcnum > 0 And GetTickCount > Player(Attacker).AttackTimer + AttackSpeed Then
       ...
       Select Case GetPlayerDir(Attacker)
           Case DIR_UP
                   ...
                   Else
                      Call MapPlayerMsg(Attacker, Trim$(Npc(npcnum).Name) & " : " & Trim$(Npc(npcnum).AttackSay), Green)
                   End If
...
' Make sure they are high enough level
    If GetPlayerLevel(Attacker) < PK_LEVEL Then
       Call MapPlayerMsg(Attacker, "Vous éte en dessous du niveau " & PK_LEVEL & ",vous ne pouvez pas encore attaquer d'autres joueurs.", BrightRed)
    Else
        If GetPlayerLevel(Victim) < NOOB_LEVEL Then
           Call MapPlayerMsg(Attacker, GetPlayerName(Victim) & " est en dessous du niveau " & NOOB_LEVEL & ",vous ne pouvez pas encore l'attaquer.", BrightRed)
        Else
            If Trim$(GetPlayerGuild(Attacker)) <> vbNullString And GetPlayerGuild(Victim) <> vbNullString Then
                If Trim$(GetPlayerGuild(Attacker)) <> Trim$(GetPlayerGuild(Victim)) Then
                    CanAttackPlayerWithArrow = True
                Else
                   Call MapPlayerMsg(Attacker, "Vous ne pouvez pas attaquer un menbre de votre guilde.", BrightRed)
                End If
            Else
                CanAttackPlayerWithArrow = True
            End If
    ...
    End If
Else
   Call MapPlayerMsg(Attacker, "La carte est une safe zone(vous ne pouvez pas attaquer d'autres joueurs).", BrightRed)
End If
