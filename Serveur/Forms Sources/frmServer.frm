   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.CheckBox Check2 
            Appearance      =   0  'Flat
            BackColor       =   &H80000005&
            Caption         =   "Autoriser l'éditeur distant"
            ForeColor       =   &H80000008&
            Height          =   345
            Left            =   120
            TabIndex        =   189
        ...
        If Command10.Caption = "Téléporter" Then
            If Index > 0 Then
                If IsPlaying(Index) Then
                   Call MapPlayerMsg(Index, "Tu as été téléporté par l'admin du serveur à la carte :" & scrlMap.value & " X:" & scrlX.value & " Y:" & scrlY.value, White)
...
For Index = 1 To MAX_PLAYERS
    If IsPlaying(Index) = True Then
        ...
        Call SendHP(Index)
       Call MapPlayerMsg(Index, "Vous avez été soigné par l'admin du serveur.", BrightGreen)
    End If
Next Index
End Sub
...
For i = 1 To lvUsers.ListItems.Count - 1 Step 1
    If lvUsers.ListItems.item(i).Selected = True Then
        Index = lvUsers.ListItems.item(i).text
       Call MapPlayerMsg(Index, "Tu peux à nouveau parler.", White)
...
For Index = 1 To MAX_PLAYERS
    If IsPlaying(Index) = True Then
        If GetPlayerAccess(Index) <= 0 Then
           ...
           Call MapPlayerMsg(Index, "Vous avez été tué par l'admin du serveur.", BrightRed)
...
If PlayerI <= MAX_PLAYERS Then
    If IsPlaying(PlayerI) Then
       ...
       Call MapPlayerMsg(PlayerI, GetPlayerName(PlayerI) & " est maintenant enregistré.", Yellow)
    End If
    PlayerI = PlayerI + 1
End If
If PlayerI >= MAX_PLAYERS Then
